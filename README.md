...
| Music | Upload your audio files (mp3/m4a/aac/wav/flac/ogg/opus), edit title and artist, search the library, and play them with seek, volume, shuffle and repeat; the waveform is drawn in the browser (click to seek) and the tempo is detected and shown in BPM; files are stored in `DATA_DIR/music` and streamed with byte ranges |
