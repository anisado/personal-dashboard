...
| Music | Upload your audio files (mp3/m4a/aac/wav/flac/ogg/opus), edit title and artist, search the library, and play them with seek, volume, shuffle and repeat; embedded tags and cover art are read on upload (the cover sits left of the waveform); the waveform is drawn in the browser (click to seek, scroll or the zoom buttons to zoom in up to 32× around the playhead) and the tempo is detected and shown in BPM with a beat/bar grid; files are stored in `DATA_DIR/music` and streamed with byte ranges |
